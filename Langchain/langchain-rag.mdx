...
flowchart LR
    subgraph INDEXING[Phase 1: Indexing - Offline]
        A[Documents] --> B[WebBaseLoader]
        B --> C[RecursiveCharacterTextSplitter]
        C --> D[HuggingFaceEmbeddings]
        D --> E[InMemoryVectorStore]
    end

    subgraph RETRIEVAL[Phase 2: Retrieval and Generation - Online]
        F[User Question] --> G[RunnableParallel]
        G --> H[Retriever]
        G --> I[RunnablePassthrough]
        H --> J[format_docs]
        J --> K[context]
        I --> L[question]
        K --> M[ChatPromptTemplate]
        L --> M
        M --> N[LLM]
        N --> O[StrOutputParser]
        O --> P[Answer]
