...
```mermaid
%%{init: {'themeVariables': {'fontSize': '10px', 'nodePadding': 5, 'textMargin': 2}}}%%
graph TD
...
graph TD
    A[Python function] -->|tool decorator| B[StructuredTool]
    B -->|bind tools| C[LLM with tools]
    C -->|tool calls| D[Tool execution]
    D -->|ToolMessage| C
    C -->|create react agent| E[CompiledStateGraph]
    E --> F[AgentState messages]
